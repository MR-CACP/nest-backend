...
describe('SessionsController', () => {
  ...
  it('revokeSession：透传 id + 操作者 ID（req.user.id）', async () => {
    ...
    await controller.revokeSession('s1', { user: { id: 'op' } } as never);
    expect(service.revokeOne).toHaveBeenCalledWith('s1', 'op');
  });

  it('revokeUserSessions：透传 id + 操作者 ID（req.user.id）', async () => {
    service.revokeAllByUser.mockResolvedValue(undefined);
    await controller.revokeUserSessions('u1', { user: { id: 'op' } } as never);
    expect(service.revokeAllByUser).toHaveBeenCalledWith('u1', 'op');
  });
});
